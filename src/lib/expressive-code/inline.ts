...
async function highlightLanguage(
  ...
  return (
    select('.ec-line .code', renderedGroupAst)?.children ?? [
      { type: 'text' as const, value: code },
    ]
  )
}

function highlightScope(ec: ExpressiveCode, code: string, scope: string) {
...
function resolveScopeColor(theme: ExpressiveCodeTheme, scope: string): string {
  ...
  return best?.foreground ?? theme.fg
}
